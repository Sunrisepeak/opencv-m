...
`mcpp.toml`(约 1500 行)承载可声明的部分:源文件 glob、公共 include 目录、51 组按 OS 归属的
`[target.'cfg(...)'.build.flags]` 逐 glob 编译选项、`[target.'cfg(os)'.build]` 的平台条件源集合、
`[features]` 定义、以及唯一的外部依赖 `compat.ffmpeg`。
...
### 3.4 平台条件编译与 Windows 的桩层

mcpp 0.0.102 起,逐 glob 的 flag 表可按平台条件化(`[target.'cfg(...)'.build.flags]`,
mcpp#258)。描述符据此按归属拆分:Linux 与 macOS 快照中逐字节一致的条目在 `cfg(unix)`,
x86_64 dispatch TU 与 nasm `.asm` 在 `cfg(linux)`,neon TU 在 `cfg(macos)`,Windows 的表在
`cfg(windows)`。谓词不匹配当前目标的条目根本不进 glob 表,因此任一平台的构建都看不到
其它平台的 glob,也不再产生结构性 dead-glob 警告(迁移前每次构建约 26 条)。

Windows 与 unix 的差异同时包含**增加与删除**(例如 zlib 组在 unix 下定义
`HAVE_UNISTD_H=1`,Windows 必须不定义)。条件条目追加在 base 之后、按 GNU last-wins
生效,删除因此可表达;当前仍沿用**桩命名空间**承载 Windows TU:每个 Windows TU 对应
`gen/windows/tu/w*/` 下的一行 `#include` 桩文件(共 703 个),为每个 TU 提供稳定的
独立路径身份。桩层的整体移除(让 Windows 直接编译 `third_party/**` 并按 cfg 撤销 unix
定义)不再受清单语法阻塞,是一项独立的后续清理。
